...
public abstract class BaseEntityRepository<T> : BaseObject where T : BaseEntity<Guid>
{
    public BaseEntityRepository() : base()
    {
        _info.Add(AppLogger.Application, "Application.Framework");
        _info.Add(AppLogger.Module, "BaseEntityRepository");
        SetupValidatorList();
    }

    public BaseEntityRepository(ILogger<BaseEntityRepository<T>> logger) : base(logger)
    {
        _info.Add(AppLogger.Application, "Application.Framework");
        _info.Add(AppLogger.Module, "BaseEntityRepository<T>");
        SetupValidatorList();
    }
        ...
        try
        {
            if (InsertValidatorList != null)
            {
                foreach (var func in InsertValidatorList)
                {
                    ...
                    errInfo.Add((await func(entityModel).ConfigureAwait(false)).Value);
                }
            }
        }
        catch (Exception ex)
    ...
    protected Guid GetModelId(string modelId)
    {
